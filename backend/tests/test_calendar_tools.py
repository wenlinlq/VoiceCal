...
async def test_add_event_and_query_structured(db_session, future_event_args):
    created = await add_calendar_event_tool(db_session, "test_openid_tools", future_event_args)
    assert created["success"] is True
    assert created["data"]["event"]["title"] == "项目会议"
    assert created["error_code"] is None

    queried = await query_calendar_events_tool(db_session, "test_openid_tools", {})
...
async def test_add_event_detects_conflict(db_session, future_event_args):
    await add_calendar_event_tool(db_session, "test_openid_tools", future_event_args)
...
async def test_duplicate_detection(db_session, future_event_args):
    await add_calendar_event_tool(db_session, "test_openid_tools", future_event_args)
    result = await detect_duplicate_events_tool(db_session, "test_openid_tools", future_event_args)
    assert result["success"] is True
...
async def test_check_time_conflict(db_session, future_event_args):
    await add_calendar_event_tool(db_session, "test_openid_tools", future_event_args)
    result = await check_time_conflict_tool(db_session, "test_openid_tools", future_event_args)
    assert result["success"] is True
...
async def test_suggest_available_slots(db_session, future_event_args):
    result = await suggest_available_slots_tool(db_session, "test_openid_tools", future_event_args)
    assert result["success"] is True
...
async def test_delete_requires_event_id(db_session):
    result = await delete_calendar_event_tool(db_session, "test_openid_tools", {})
    assert result["success"] is False
...
async def test_add_event_requires_confirm(db_session, future_event_args):
    ...
    result = await add_calendar_event_tool(db_session, "test_openid_tools", args)
    assert result["success"] is False
    assert result["error_code"] == "CONFIRM_REQUIRED"
    assert result["data"]["preview"]["title"] == "项目会议"

    # 确认没有实际创建
    query_result = await query_calendar_events_tool(db_session, "test_openid_tools", {})
...
async def test_add_event_with_confirm_succeeds(db_session, future_event_args):
    ...
    result = await add_calendar_event_tool(db_session, "test_openid_tools", future_event_args)
    assert result["success"] is True
...
async def test_delete_event_requires_confirm(db_session, future_event_args):
    ...
    result = await delete_calendar_event_tool(db_session, "test_openid_tools", {"event_id": event_id})
    assert result["success"] is False
    assert result["error_code"] == "CONFIRM_REQUIRED"
    assert result["data"]["preview"]["title"] == "项目会议"
    ...
    # 带 confirm → 真正删除
    result2 = await delete_calendar_event_tool(db_session, "test_openid_tools", {"event_id": event_id, "confirm": True})
    assert result2["success"] is True

    query_result2 = await query_calendar_events_tool(db_session, "test_openid_tools", {})
...
async def test_query_by_keyword(db_session):
    ...
    await add_calendar_event_tool(db_session, "test_openid_tools", {
        "title": "七牛云面试",
    ...
    # 关键词"七牛云"应匹配标题和描述
    result = await query_calendar_events_tool(db_session, "test_openid_tools", {"keyword": "七牛云"})
    ...
    # 关键词"健身"只匹配一个
    result2 = await query_calendar_events_tool(db_session, "test_openid_tools", {"keyword": "健身"})
    assert result2["data"]["count"] == 1

    # 不存在的关键词
    result3 = await query_calendar_events_tool(db_session, "test_openid_tools", {"keyword": "不存在"})
...
async def test_get_event_by_id(db_session):
    ...
    t = datetime.now(timezone.utc) + timedelta(days=1)
    created = await add_calendar_event_tool(db_session, "test_openid_tools", {
        "title": "测试日程",
    ...
    result = await get_event_by_id_impl(db_session, "test_openid_tools", {"event_id": event_id})
    assert result["success"] is True
    assert result["data"]["event"]["title"] == "测试日程"

    # 不存在的 ID
    fake = await get_event_by_id_impl(db_session, "test_openid_tools", {"event_id": "00000000-0000-0000-0000-000000000000"})
