    ...
    <view v-else class="list-body">
      ...
      >
        <view class="time-col">
          ...
          <text v-if="event.end_time" class="time-end">{{ formatTime(event.end_time) }}</text>
        </view>
        <view class="info-col">
          <text class="event-title">{{ event.title }}</text>
          <text v-if="event.repeat_type && event.repeat_type !== 'none'" class="repeat-badge">
...
const headerTitle = computed(() => {
  ...
  return props.currentDate === todayStr ? '今日事件' : `${formatDisplayDate(props.currentDate)} 事件`
})
